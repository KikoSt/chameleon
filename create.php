...
sendCreatives($companyId, $advertiserId, $categoryId);
...
function sendCreatives($companyId, $advertiserId, $categoryId)
{
    ...
    foreach(glob($basePath . "*.swf") AS $cur)
    {
        ...
        list($null, $productId, $null, $size) = explode('_', $filename);
        unset($null);
        ...
        $curBanner->setProductId($filename);
        $curBanner->setWidth($width);
        $curBanner->setHeight($height);

        $connector->sendCreative($curBanner);
        die();
